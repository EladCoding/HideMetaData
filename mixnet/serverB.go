...
type ServerListener002 struct {
	name string
}

// Rpc method that receiving a message from the mixnet distributor.
func (l *ServerListener002) GetMessage(msg OnionMessage, reply *EncryptedMsg) error {
	from := msg.From
...
func (l *ServerListener002) listenToMyAddress() {
...
// Start a server node.
func StartServer002(name string) {
	log.Printf("Starting Server %v...\n", name)
	listener := ServerListener002{name}
