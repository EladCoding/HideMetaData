...
type ServerListener001 struct {
	name string
}

// Rpc method that receiving a message from the mixnet distributor.
func (l *ServerListener001) GetMessage(msg OnionMessage, reply *EncryptedMsg) error {
	from := msg.From
...
func (l *ServerListener001) listenToMyAddress() {
...
// Start a server node.
func StartServer001(name string) {
	log.Printf("Starting Server %v...\n", name)
	listener := ServerListener001{name}
